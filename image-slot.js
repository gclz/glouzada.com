 ...
 *
 * Drop this into a page wherever you want the user to supply an image.
 * The dropped image persists across reloads via localStorage.
 *
 * Attributes:
...
(() => {
  ...
  function getStored(id) {
    if (!id) return null;
    try {
      const v = localStorage.getItem('image-slot:' + id);
      if (!v) return null;
      const parsed = JSON.parse(v);
      if (parsed && parsed.u && !/^data:image\//i.test(parsed.u)) return null;
      return parsed;
    } catch { return null; }
  }

  function setStored(id, val) {
    if (!id) return;
    try {
      if (val) localStorage.setItem('image-slot:' + id, JSON.stringify(val));
      else localStorage.removeItem('image-slot:' + id);
    } catch {}
  }
  ...
  class ImageSlot extends HTMLElement {
    ...
    _commitView() {
      const stored = getStored(this.id) || {};
      const v = { u: stored.u || null, s: this._view.s, x: this._view.x, y: this._view.y };
      if (this.id) setStored(this.id, v);
    }
    ...
    _render() {
      ...
      let stored = this.id ? getStored(this.id) : this._local;
